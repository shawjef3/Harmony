...
trait ScalazMonadReader[F[_], S]
  extends ScalazApplicative[F]
    with ScalazBindRec[F] {
  self: scalaz.MonadReader[F, S] with scalaz.BindRec[F] =>

  protected implicit def catsMonadReader: cats.MonadReader[F, S]
  override protected implicit lazy val catsApplicative: cats.Applicative[F] = catsMonadReader
  override protected implicit lazy val catsFlatMap: cats.FlatMap[F] = catsMonadReader

  override def ask: F[S] =
    catsMonadReader.ask

  override def local[A](f: (S) => S)(fa: F[A]): F[A] =
    catsMonadReader.local(f)(fa)

}

trait MonadReaderConverter {

  implicit def catsToScalazMonadReader[F[_], S](implicit inner: cats.MonadReader[F, S]): scalaz.MonadReader[F, S] =
    new ScalazMonadReader[F, S] with scalaz.MonadReader[F, S] with scalaz.BindRec[F] {
  ...
  implicit def catsToScalazMonadReaderValue[F[_], S](inner: cats.MonadReader[F, S]): scalaz.MonadReader[F, S] =
    catsToScalazMonadReader[F, S](inner)
...
trait ScalazMonadState[F[_], S]
  ...
  protected implicit def catsMonadState: cats.MonadState[F, S]
  override protected implicit lazy val catsApplicative: cats.Applicative[F] = catsMonadState
  override protected implicit lazy val catsFlatMap: cats.FlatMap[F] = catsMonadState
...
trait MonadStateConverter {
  ...
  implicit def catsToScalazMonadState[F[_], S](implicit inner: cats.MonadState[F, S]): scalaz.MonadState[F, S] =
  ...
  implicit def catsToScalazMonadStateValue[F[_], S](inner: cats.MonadState[F, S]): scalaz.MonadState[F, S] =
